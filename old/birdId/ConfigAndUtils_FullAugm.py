...
if SampleRate == 22050:
    AudioFilesDcaseDir = (
        RootDir + "BAD_Challenge_18/AudioFilesPreprocessed/03_3_Hp2000_22050Hz_Norm/"
    )
else:
    AudioFilesDcaseDir = (
        RootDir
        + "KaggleBirdsongRecognition2020/data/audio/13_wav_filtered_and_normalized_32000Hz_kaiser_fast/"
    )
...
ClassIds = np.loadtxt(
    RootDir + "KaggleBirdsongRecognition2020/metadata/classIds.txt",
    dtype=np.str,
    delimiter="\t",
).tolist()
...
trainSet_df = pd.read_csv(
    RootDir
    + "KaggleBirdsongRecognition2020/metadata/trainSet_df_05_NoBgSpecies_32000Hz.csv",
    sep=";",
)
...
noise_df = pd.read_csv(
    RootDir + "KaggleBirdsongRecognition2020/metadata/noiseSet_df_01_32000Hz.csv",
    sep=";",
)
...
def getAraiBaselineTrainValSplit():
    ...
    train_df_new = pd.read_csv(
        RootDir + "KaggleBirdsongRecognition2020/metadata/train.csv"
    )
    ebird_code = "lotduc"
    filename = "XC195038.mp3"
    train_df_new = train_df_new[
        ~(
            (train_df_new["ebird_code"] == ebird_code)
    ...
    # Use StratifiedKFold
    splitter = sms.StratifiedKFold(n_splits=1, random_state=42, shuffle=True)
